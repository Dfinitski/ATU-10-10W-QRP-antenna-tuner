...
char btn_1_cnt = 0;
unsigned long volt_cnt = 0, watch_cnt = 0, btn_cnt = 0, off_cnt, sleep_cnt, tune_cnt;
int PWR, SWR, SWR_fixed_old = 9999, PWR_fixed_old = 9999;
char ind = 0, cap = 0, SW = 0, step_cap = 0, step_ind = 0, L_linear = 0, C_linear = 0;
bit Sleep, Overflow, B_short, B_long, B_xlong;

int Rel_Del = 5, min_for_start = 10, max_for_start = 150;
int Auto_delta = 130; // 1.3 SWR

unsigned long Sleep_time = 300; // 300   Time in seconds
unsigned long Off_time = 1800; // 1800   Time in seconds
...
void interupt() iv 0x0004  {
   ...
   if(IOCBF5_bit){
      IOCBF5_bit = 0;
      sleep_cnt = Tick + Sleep_time*1000;
   }
   if(Tick>=btn_cnt){  // every 10ms
      btn_cnt += 10;
      //
      if(GetButton){  //
         if(btn_1_cnt<255) btn_1_cnt ++;
         if(btn_1_cnt==255) B_xlong = 1;  //  Xtra long pressing detected
      }
      else if(btn_1_cnt>=35 & btn_1_cnt<255){
         B_long = 1;
         btn_1_cnt = 0;
      }
      else if(btn_1_cnt>10 & btn_1_cnt<35){
...
void main() {
   ...
   //
   while(1) {
      if(Tick>=volt_cnt & Sleep==0){   // every 3 second
         volt_cnt += 3000;
         Voltage_show();
      }
      //
      if(Tick>=watch_cnt & Sleep==0){   // every 300 ms
         watch_cnt += 300;
         watch_swr();
         if(B_short) Btn_short();
         if(B_long) Btn_long();
         if(B_xlong) Btn_xlong();
      }
      //
      if(Tick>=sleep_cnt & Sleep==0){  // Go to sleep
         Sleep = 1; 
         sleep_mode();
      }
      //
      ...
      //
      if(Tick>=off_cnt & Sleep){    // one hour after sleep
         power_off();
      }
      //
...
void Sleep_mode(){
   if(Sleep){    // go to sleep
      OLED_PWD = 0;
      OSCFRQ = 0b00000000; // 1MHz
      OSCCON1 = 0b01100101; // /32
      off_cnt = Tick + Off_time;
      ADON_bit = 0;
      FVREN_bit = 0;
      FVRMD_bit = 1; // Disable FVR
      ADCMD_bit = 1; // Disable ADC
   }
   else{  // return to work from sleep
      FVRMD_bit = 0; // Enable FVR
      ADCMD_bit = 0; // Enable ADC
      ADON_bit = 1;
      FVREN_bit = 1;
      OSCFRQ = 0b00000110; // 32MHz
      OSCCON1 = 0b01100000; // /1
      oled_start();
      volt_cnt = Tick;
      watch_cnt = Tick;
      Sleep_cnt = Tick + Sleep_time*1000;
...
void watch_swr(void){
   ...
   if(SWR_fixed>=Auto_delta & ((SWR_fixed>SWR_fixed_old & (SWR_fixed-SWR_fixed_old)>delta) | (SWR_fixed<SWR_fixed_old & (SWR_fixed_old-SWR_fixed)>delta) | SWR_fixed_old==999))
      { Tune(); Voltage_old = 9999; volt_cnt = Tick; watch_cnt = Tick; }
   // asm NOP;
...
void Voltage_show(){
   ...
   if(Voltage<3400){
      ...
      Sleep = 1;
      Sleep_mode();
      off_cnt = Tick; // go to power off  immediatelly after sleep
   }
   return;
}
...
void power_off(void){
   ...
   btn_cnt = 0;
   while(1){
      if(btn_cnt==0) { Delay_us(100); IOCBF5_bit = 0; asm sleep; }
      asm NOP;
      Delay_us(100);
